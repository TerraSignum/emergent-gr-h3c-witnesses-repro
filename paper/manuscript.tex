...
\newcommand{\repoOwn}{emergent-gr-h3c-witnesses-repro}
...
\end{itemize}
Bundle:
\pyown{src/verify_chirality_sup_DOmega.py}{verify_chirality_sup_DOmega}, output
\pyown{outputs/verify_chirality_sup_DOmega.json}{verify_chirality_sup_DOmega.json}.
...
\paragraph{Structural argument and branch-resolved
empirical readout.}
...
$w_{l,i}\!=\!\sum_{k=3}^{5}\!V[i,k]^{2}$. The structural
heuristic identifies each squared eigenvector amplitude
$V[i,k]^{2}$ with a propagator weight carrying one factor
of the Yukawa-damping survival amplitude
$\alpha_{\xi}\!=\!1-\gamma$ (the universal P3 loop-class
factor underlying the cluster $1\pm\gamma/4$ family); the
sum $w_{q}+w_{l}$ then saturates at $\alpha_{\xi}$ while
the \emph{difference} $w_{q}-w_{l}$, vanishing on the
chirality-symmetric state, is suppressed by an additional
factor $\alpha_{\xi}$, yielding the structural rate
$S(N)\!\sim\!N^{-\alpha_{\xi}^{2}}$ with
$\alpha_{\xi}^{2}\!=\!81/100$. The same identity
...
identification reported above.
The canonical-physics ladder
$N\!\in\!\{50,64,72,84,100,128,200,256,300,512\}$ ($10$
regimes) spans the matter--vacuum chirality flip at
$N^{*}\!\approx\!110$--$120$
(where $\theta_{\mathrm{chir}}\!=\!\pi/4$;
$\theta_{\mathrm{chir}}\!=\!37.1^{\circ}$ at $N\!=\!100$
versus $46.1^{\circ}$ at $N\!=\!128$, parent's per-regime
causal-wave readout), so the chirality-sup decay is
branch-resolved.
\begin{equation*}
\begin{array}{lll}
\text{Vacuum (}N\!\le\!100,\,5\text{ pts):} & \hat\alpha^{\rm vac}\!=\!0.7425, & R^{2}\!=\!0.94 \\
\text{Matter (}N\!\ge\!128,\,5\text{ pts):} & \hat\alpha^{\rm mat}\!=\!0.3861, & R^{2}\!=\!0.68 \\
\text{Pooled (}10\text{ pts):} & \hat\alpha\!=\!0.6445, & R^{2}\!=\!0.93 \\
\end{array}
\end{equation*}
The vacuum-branch fit lies $8.3\%$ below $\alpha_{\xi}^{2}\!=\!81/100$
and the pooled fit lies $3.3\%$ below $2/3\!=\!(d{-}2)/(d{-}1)$;
the bootstrap-CI$_{95\%}$ $[0.60, 0.83]$ on the $10$-regime
pooled $\hat\alpha$ contains both candidates, and the AICc
model comparison places $2/3$ as the formal best, with
$\alpha_{\xi}^{2}$ at $\Delta\mathrm{AICc}\!=\!+2.35$. The
matter-branch $\hat\alpha\!=\!0.39$ at $R^{2}\!=\!0.68$
reflects the breakdown of clean power-law decay post-flip,
analogous to the matter-branch behaviour of the
Lemma~B uniform-spectral-gap audit in the parent
paper~\cite{Paper4}. The cross-observable identification
with $\Lambda_{t}^{\rm mat}\!=\!81/100$ remains a candidate
structural anchor on the vacuum side; matter-branch
universality of the $\alpha_{\xi}^{2}$ rate is currently
\emph{open} pending higher-$N$ data, and a clean
analytical version of the sketch (specific propagator-step
identification, explicit sup-saturation bound, branch-
resolved continuum-limit identification) remains a
structural follow-up.

\begin{table}[!htbp]
\centering\footnotesize
\input{tables/tab_alpha_model_comparison.tex}
...
\pyown{src/verify_chirality_to_integer_lift.py}{verify_chirality_to_integer_lift}, output
\pyown{outputs/verify_chirality_to_integer_lift.json}{verify_chirality_to_integer_lift.json}.
